...
struct IController
{
  ...
  struct
  {
    std::function< void(std::string pwd)> passwordEnterred;
  ...
  {
    if (! in.passwordEnterred) throw dzn::binding_error(meta, "in.passwordEnterred");
...
#define ENUM_Controller_State 1
  ...
  private:
  void iController_passwordEnterred(std::string pwd);
